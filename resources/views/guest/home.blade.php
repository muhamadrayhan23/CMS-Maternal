...
<div id="default-carousel" data-carousel="slide">
    <div class="relative w-full h-screen font-sans -mt-20 overflow-hidden">
        @foreach($banners as $index => $banner)
            <div class="hidden duration-700 ease-in-out" data-carousel-item>
                <img src="{{ asset('img/' . $banner->banner_image) }}" 
    ...
    <div class="absolute z-30 flex -translate-x-1/2 bottom-5 left-1/2 space-x-3 rtl:space-x-reverse">
        @foreach($banners as $index => $banner)
            <button type="button" 
                    class="w-3 h-3 rounded-full {{ $index == 0 ? 'bg-white' : 'bg-white/60' }}" 
    ...
    <button type="button" class="absolute top-0 start-0 z-30 flex items-center justify-center h-full px-4 cursor-pointer group focus:outline-none" data-carousel-prev>
        <span class="inline-flex items-center justify-center w-10 h-10 rounded-full border border-white group-hover:bg-white/50 dark:group-hover:bg-gray-800/70 group-hover:border-gray-800/70">
