...
@section('content')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 lg:gap-16">
            ...
            <div class="flex flex-col gap-4 md:sticky md:top-24 self-start">
                ...
                <nav class="flex items-center justify-between text-xs md:text-sm text-gray-400 md:mt-10">
                    ...
                    <div id="variant-name" class="hidden md:block text-black text-xs md:text-sm font-medium tracking-wide">
                        {{ $product->details->first()->atribute_name ?? '' }}
                    </div>
                </nav>

                <div class="flex flex-col md:flex-row gap-2 items-start">
                    <div
                        class="order-1 md:block md:order-2 w-full flex-1 bg-gray-100 rounded-sm overflow-hidden aspect-square">
                        <img id="main-image" src="{{ asset($product->details->first()->image_product) }}"
                    ...
                    <div class="order-2 md:order-1 relative self-center px-4">
                        ...
                        <div id="thumbs"
                            class="flex flex-row md:flex-col gap-3 max-w-[370px] md:max-w-none
                    ...
                    px-1 py-2">
                            ...
                            {{ $index === 0 ? 'ring-2 ring-[#ff5e00]' : 'hover:ring-2 hover:ring-[#ff5e00]' }}"
                                    data-key="{{ $index }}" data-src="{{ asset($detail->image_product) }}"
                                    data-name="{{ $detail->atribute_name }}">

                                    <img src="{{ asset($detail->image_product) }}"
            ...
            <div id="variant-name-mobile" class="block items-start md:hidden text-xs font-medium text-black tracking-wide">
                {{ $product->details->first()->atribute_name ?? '' }}
            </div>
            ...
            <div class="self-start md:mt-10">
                ...
                {{-- Bagian Description Tetap Dipertahankan --}}
                <div class="relative">
                    <p id="product-desc"
                    ...
                    <button id="read-more-btn" class="mt-2 text-xs md:text-sm font-medium text-black underline">
                        Read more
                    </button>
                </div>
                ...
                <div class="flex flex-col gap-5 mt-8 mb-8">
                    ...
                    <div class="flex flex-row gap-4 flex-wrap">
                        ...
                        border border-black text-black md:rounded-sm"
                                data-key="{{ $index }}" data-src="{{ asset($p->image_product) }}"
                                data-name="{{ $p->atribute_name }}" data-price="{{ $p->price ?? $product->price }}">


                                <img src="{{ asset($p->image_product) }}" alt="" class="w-5 h-5 md:w-8 md:h-8">
                                <span class="text-sm">
                                    {{ $p->atribute_name }}
                                </span>
                            </button>
    ...
    <div class="mx-auto px-4 md:px-10 mb-10">
        <div id="product-cards" class="flex flex-col sm:grid sm:grid-cols-2 lg:flex lg:flex-row lg:flex-wrap gap-10">

            @foreach ($products as $product)
                <div class="w-full sm:w-auto lg:w-[23%] transition-all duration-300 hover:scale-[1.03]">

                    <div
                        class="aspect-square transition-all duration-300 ease-out hover:scale-105 relative cursor-pointer">
...
{{-- Modal and Scripts tetap sama seperti versi sebelumnya --}}
...
<div id="image-modal" class="fixed inset-0 bg-black/80 hidden items-center justify-center z-[9999]">
    ...
    <img id="modal-image" class="max-w-[90vw] max-h-[90vh] object-contain cursor-zoom-out">
