<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 font-sans">
            ...
            <div class="p-3 md:p-4">
                <div class="flex justify-between items-start mb-2">
                    ...
                    <div class="relative">
                        ...
                        <div
                            class="action-menu hidden absolute right-0 mt-2 w-30 bg-white border border-gray-200 rounded-xl shadow-lg z-50 overflow-hidden text-left">
                            <form method="POST" action="{{ route('banner.toggle', $b->id_banner) }}">
                                @csrf
                                @method('PATCH')
                                <button
                                    class="w-full px-4 py-3 text-sm hover:bg-gray-200 transition-all flex gap-2.5 text-left">
                                    @if ($b->is_active)
                            ...
                            <a href="{{ route('editB', $b->id_banner) }}"
                ...
                <div class="flex justify-between items-center mt-6 text-gray-600">
                    <div class="text-xs">By <span
                            class="font-semibold text-gray-700">{{ $b->user->name ?? 'Admin' }}</span></div>
    ...
    @empty
        <div
            class="col-span-full text-center py-10 text-gray-500 bg-white rounded-2xl border border-dashed border-gray-300">
            @if ($search)
                <span class="font-bold">"{{ $search }}"</span> not found.
            @elseif($status !== null && $status !== '')
                There is no banner with that status <span
                    class="font-bold">{{ $status == 1 ? 'Published' : 'Unpublished' }}</span>
            @else
