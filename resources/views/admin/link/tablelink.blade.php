                ...
                @forelse ($links as $link)
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="p-2 whitespace-nowrap">
                            <span
                                class="text-green-700 bg-green-100/90 text-[10px] py-1.5 px-3 md:px-5 rounded-full font-medium">
                                Published
                            </span>
                        </td>
                        <td class="p-3 relative">
                            <div class="relative inline-block">
                                <button onclick="toggleMenu(this)" class="text-gray-400 hover:text-gray-600 p-1">
                                ...
                                <div
                                    class="action-menu hidden absolute right-0 mt-2 w-40 bg-white border border-gray-200 rounded-xl shadow-xl z-[100]">
                                    <a href="{{ route('editLink', $link->id_link) }}"
                                        class="flex items-center gap-3 px-4 py-3 text-sm hover:bg-gray-200 transition-all">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14"
                                            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                            stroke-linecap="round" stroke-linejoin="round">
                                            <path d="M13 21h8" />
                                    ...
                                    <form id="delete-form-{{ $link->id_link }}" method="POST"
                                        action="{{ route('deleteLink', $link) }}">
                                        @csrf @method('DELETE')
                                        <button type="button" data-id="{{ $link->id_link }}"
                                            class="btn-delete flex items-center gap-3 w-full px-4 py-3 text-sm text-black hover:bg-gray-200 transition-all text-left border-t border-gray-50">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14"
