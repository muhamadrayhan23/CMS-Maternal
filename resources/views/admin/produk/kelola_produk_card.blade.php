...
<div class="bg-gray-100 min-h-screen">
   ...
   {{ request()->routeIs('produk.create') ? 'bg-gray-700 text-white' : 'bg-white text-gray-800' }}">
        ...
        <form method="GET" action="{{ route('produk.kelola_card') }}" class="mb-6">

            <div class="grid grid-cols-1 md:grid-cols-12 gap-3">

                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search product..."
                    class="md:col-span-9 px-4 py-2 rounded border bg-white
focus:ring-2 focus:ring-gray-400"
                    oninput="submitFilter()">

                <select name="status" onchange="submitFilter()"
                    class="md:col-span-3 px-4 py-2 rounded border bg-white">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            @foreach ($produk as $p)
                <div
                    ...
                    <div class="absolute top-3 right-3 relative">
                        <button onclick="toggleMenu(this)"
                            class="w-8 h-8 bg-white border rounded flex items-center justify-center hover:bg-gray-100">
                            ⋮
                        </button>

                        <div
                            class="action-menu hidden absolute right-0 mt-2 w-44 bg-white border rounded-lg shadow-lg z-40">
...
<script>
    ...
    function submitFilter() {
        ...
        timeout = setTimeout(() => document.forms[0].submit(), 400)
    }
